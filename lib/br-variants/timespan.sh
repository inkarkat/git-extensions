...
@(correct|fix@(up|amend|wording))|commit@(identical|like|relate|continue)|amendrelate\
	...
	;;
    bisectthosefiles)
	filesCommand --source-command "$scope files --revision REVISION " bisect start "$@";;
    ...
    lghithosefiles)
	filesCommand --source-command "$scope files --revision REVISION" "${gitCommand%thosefiles}files" "$@";;
