...
lh?(mine|others|team)|\
@(l?(o)g?([fv]|merges)|l?(o)|count|countmaxdaycommits|commitsperday|logdistribution)?(mine|others|team)|\
...
lc@(?(l)?(f)|?(f))?(except)by|\
@(l?(o)g?([fv]|merges)|l?(o)|count|countmaxdaycommits|commitsperday|logdistribution)?(except)by|\
activity?(except)by\
)
